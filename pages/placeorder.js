...
export default function PlaceOrder() {
    ...
    if (orderId) {
      const options = {
        ...
        "handler": function (response) {
          ...
          fetch(`https://equipment-renting.herokuapp.com/payment/verification`, {
            method: "POST",
            headers: {
              'Content-Type': 'application/json',
            },
  ...
  return <div className={orderStyles.paymentcheckout}>
    
    {paymentStatus ? <div><p>{paymentStatus}, your order ID: {orderId}. </p><p>Thanks for shopping with us!! 😀</p></div> : <div>
    {items.map((item,index) => <div className={orderStyles.itempay} key={index}><div className={orderStyles.itempayable}>{item.name}</div>
    <div>₹ {item.price} X {days}</div></div>)}

    <div><p>Total Amount Payable: <span style={{marginLeft: '67px'}}>₹ {amount}</span></p> 
    <ColorButton style={{marginLeft: '210px',marginTop:'15px'}} onClick={handleOrder} variant="contained">{confirm ? 'PAY' : 'confirm'}</ColorButton></div>
    </div>}
  </div>;
