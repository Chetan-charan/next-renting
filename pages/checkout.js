...
export default function CheckOut() {
  ...
  const onChange = (dateRange) => {
    ...
    dispatch(updateDays(Math.round(Math.abs((dateRange[1] - dateRange[0]) / (24 * 60 * 60 * 1000)))));
  };

  return <div className={checkoutStyles.orderitemlist}>
    <div>
    <h3>Ordered Items</h3>
    <List sx={{ width: '100%', maxWidth: 360, bgcolor: 'background.paper' }}>
      {items.map((item) => <ListItemEach key={item.name} name={item.name} pic={item.pic} price={item.price} />)}
    </List>
    ...
    <div>
      ...
      <Calendar selectRange minDate={new Date()} onChange={onChange} value={dateRange} />
